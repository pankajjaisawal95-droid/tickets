...
/* Required fields for an event (validated before save). */
const REQUIRED = {
  title: 'Title',
  venue: 'Venue',
...
const BLANK = {
  ...
  category_id: '', organizer_id: '', start_datetime: '', end_datetime: '',
  approval_status: 'PENDING', is_active: 1,
...
export default function EventForm() {
  ...
  const { data: cats } = useFetch(() => get('/admin/categories'));
  const { data: organizers } = useFetch(() => get('/admin/organizers'));
  const { data, loading, error, reload } = useFetch(
    () => (isEdit ? get(`/admin/events/${id}`) : Promise.resolve(null)),
    [id]
  );
  ...
  useEffect(() => {
    if (data?.event) {
      ...
      setForm({
        ...
        category_id: e.category_id || '', organizer_id: e.organizer_id || '',
        start_datetime: toLocalInput(e.start_datetime), end_datetime: toLocalInput(e.end_datetime),
        approval_status: e.approval_status || 'PENDING', is_active: e.is_active ? 1 : 0,
  ...
  const save = async (e) => {
    ...
    try {
      if (isEdit) {
        await put(`/admin/events/${id}`, payload);
  ...
  // the lists just refresh in place when the new data arrives.
    ...
    <div>
      ...
      <form className="panel" style={{ padding: 20, marginBottom: 22 }} onSubmit={save}>
        <div className="form-grid">
          ...
          <label className="field">
            <span>Organizer <span className="req">*</span></span>
            <select className={`input ${errors.organizer_id ? 'invalid' : ''}`} value={form.organizer_id} onChange={(e) => set('organizer_id', e.target.value)}>
              <option value="">— Select —</option>
              {(organizers || []).map((o) => (
                <option key={o.id} value={o.id}>
                  {o.organization_name || o.user_name || `Organizer #${o.id}`}
                </option>
              ))}
            </select>
            {errors.organizer_id && <span className="err-text">{errors.organizer_id}</span>}
          </label>
          <label className="field">
...
/* -------------------------- seating layout (CSV) -------------------------- */
// Ready-to-edit template (Mavalankar CCI layout). "|" = aisle gap between blocks.
const SEAT_TEMPLATE_CSV = [
  'Section,Row,Blocks',
  'Platinum,A,1-7|8-14',
  'Platinum,B,1-4|5-11|12-18|19-22',
  'Diamond,C,1-5|6-14|15-23|24-28',
  'Diamond,D,1-5|6-14|15-23|24-28',
  'Diamond,E,1-5|6-14|15-23|24-28',
  'Gold,F,1-5|6-14|15-23|24-28',
  'Gold,G,1-5|6-14|15-23|24-28',
  'Gold,H,1-4|5-13|14-22|23-26',
  'Silver,I,1-6|7-15|16-24|25-30',
  'Silver,J,1-6|7-15|16-24|25-30',
  'Silver,K,1-6|7-15|16-24|25-30',
  'Silver,L,1-6|7-15|16-24|25-30',
  'Silver,M,1-6|7-15|16-24|25-30',
  'Silver,N,1-5|6-14|15-19',
  'Bronze,O,1-10|11-20|21-30',
  'Bronze,P,1-10|11-20|21-30',
  'Bronze,Q,1-10|11-21|22-31',
  'Bronze,R,1-10|11-21|22-31',
  'Bronze,S,1-10|11-21|22-31',
  'Bronze,T,1-10|11-22|23-32',
  'Bronze,U,1-10|11-20|21-30',
].join('\n');
...
function SeatLayoutImport({ eventId, reload }) {
  ...
  const onImport = async () => {
    ...
    try {
      ...
      const res = await post(`/admin/events/${eventId}/seatmap/import`, { csv });
...
function TicketTypes({ eventId, types, reload }) {
  ...
  const onSave = async (vals) => {
    ...
    try {
      ...
      if (editing === 'new') await post(`/admin/events/${eventId}/ticket-types`, payload);
      else await put(`/admin/ticket-types/${editing.id}`, payload);
      toast.success('Saved');
      setEditing(null);
      reload();
    } catch (err) { toast.error(err.message); } finally { setBusy(false); }
  ...
  const onDelete = async () => {
    setBusy(true);
    try { await del(`/admin/ticket-types/${confirm.id}`); toast.success('Removed'); setConfirm(null); reload(); }
    catch (err) { toast.error(err.message); } finally { setBusy(false); }
  };
...
function SeatGridManager({ typeId, onGenerated }) {
  ...
  const loadSeats = async () => {
    if (!typeId) return;
    try {
      const res = await get(`/admin/ticket-types/${typeId}/seats`);
  ...
  const generate = async () => {
    ...
    setLoading(true);
    try {
      const res = await post(`/admin/ticket-types/${typeId}/seats/generate`, { rows: r, cols: c });
  ...
  const saveSeat = async () => {
    ...
    setSaving(true);
    try {
      await put(`/admin/seats/${selected.id}`, {
  ...
  const toggleSeat = async (s) => {
    setSaving(true);
    try {
      await put(`/admin/seats/${s.id}`, { status: s.status ? 0 : 1 });
      toast.success(s.status ? 'Seat disabled' : 'Seat enabled');
      // Backend keeps ticket_type.total_quantity in sync with active seats.
      setSelected(null);
      await loadSeats();
    } catch (err) { toast.error(err.message); } finally { setSaving(false); }
  };
  ...
  const renameRow = async (from, to) => {
    ...
    setSaving(true);
    try {
      await put(`/admin/ticket-types/${typeId}/seats/rename-row`, { from, to: t });
  ...
  return (
    <div className="panel" style={{ padding: 18, marginBottom: 22 }}>
      ...
      <MediaList
        ...
        onAdd={(b) => post(`/admin/event/${eventId}/gallery`, b)}
        onUpdate={(id, b) => put(`/admin/event/gallery/${id}`, b)}
        onDelete={(id) => del(`/admin/event/gallery/${id}`)}
        onReorder={(items) => put('/admin/event/gallery/reorder', { items })}
        reload={reload}
      />
      <MediaList
        title="Artists"
        ...
        onAdd={(b) => post(`/admin/event/${eventId}/artist`, b)}
        onUpdate={(id, b) => put(`/admin/event/artist/${id}`, b)}
        onDelete={(id) => del(`/admin/event/artist/${id}`)}
        onReorder={(items) => put('/admin/event/artist/reorder', { items })}
        reload={reload}
      />
    </div>
  );
...
function MediaList({ title, rows, cols, addFields, onAdd, onUpdate, onDelete, onReorder, reload }) {
  ...
  // Renders the editable fields (image upload + text inputs) for a draft object.
  // Shared by the Add form and the inline Edit panel so both stay in sync.
