...
export const listOrganizersService = async () => {
  const [rows] = await pool.query(
    `SELECT o.id, o.organization_name, o.contact_email, o.kyc_status, o.status, o.created_at,
            o.kyc_submitted_at, o.pan, o.gst_number,
            o.bank_account_holder, o.bank_account_number, o.bank_ifsc, o.bank_name,
            o.pan_doc_url, o.gst_doc_url,
...
/** Admin KYC decision for an organiser: APPROVED | REJECTED | PENDING. */
const KYC_STATUSES = new Set(["PENDING", "APPROVED", "REJECTED"]);
export const setOrganizerKycStatusService = async (id, kyc_status) => {
  const next = String(kyc_status || "").toUpperCase();
  if (!KYC_STATUSES.has(next)) throw new Error("Invalid kyc_status (PENDING | APPROVED | REJECTED)");
  const [res] = await pool.query(
    `UPDATE organizers SET kyc_status = ? WHERE id = ?`,
    [next, id]
  );
  if (res.affectedRows === 0) throw new Error("Organizer not found");
  return { id: Number(id), kyc_status: next };
};
