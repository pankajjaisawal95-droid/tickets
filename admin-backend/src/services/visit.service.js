...
export const ensureVisitLogTable = async () => {
  try {
    ...
    // Backfill the geolocation columns for installs created before they existed.
    await ensureColumn("visit_logs", "latitude", "DECIMAL(10,7) NULL AFTER city");
    await ensureColumn("visit_logs", "longitude", "DECIMAL(10,7) NULL AFTER latitude");
    await ensureColumn("visit_logs", "address", "VARCHAR(255) NULL AFTER longitude");
 ...
 */
export const logVisit = async ({ geo, location = {}, path = null, userAgent = null, userId = null }) => {
  try {
    const [result] = await pool.query(
      `INSERT INTO visit_logs
         (user_id, ip, country, region, city, latitude, longitude, address, path, user_agent)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        userId,
        geo?.ip || null,
        ...
        location.city || null,
        Number.isFinite(location.latitude) ? location.latitude : null,
        Number.isFinite(location.longitude) ? location.longitude : null,
...
export const listVisits = async ({ limit = 20, offset = 0, country, search } = {}) => {
  ...
  const [rows] = await pool.query(
    `SELECT id, user_id, ip, country, region, city, latitude, longitude, address, path, user_agent, created_at
     FROM visit_logs
     ${whereSql}
