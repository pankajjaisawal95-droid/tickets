...
import { formatTicketNo } from '../utils/ticketNo.js';
  ...
  <tr>
    <td style="padding:0 30px;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:16px;border:1px solid #e5e7eb;">
        ...
        <tr><td style="padding:8px 20px;color:#6b7280;">Booking ID</td><td style="padding:8px 20px;font-weight:600;color:#111827;">${esc(formatTicketNo(ticket?.ticketId, ticket?.createdAt))}</td></tr>
...
export const sendTicketEmail = async ({ contact, event, ticket, breakdown = [], meta = {} }) => {
  ...
  try {
    ...
    if (pdf?.length) {
      attachments.push({
        filename: `${formatTicketNo(ticket?.ticketId, ticket?.createdAt) || 'ticket-booking'}.pdf`,
        content: pdf,
  ...
  const text =
    `Your ticket is confirmed (Booking ${formatTicketNo(ticket?.ticketId, ticket?.createdAt)})\n\n` +
    `Event: ${event?.title}\nVenue: ${event?.venue}\n` +
...
export const sendCancellationEmail = async ({ contact, event, ticket, breakdown = [], refund = null, meta = {} }) => {
  ...
  <div style="background:#0b0b12;padding:24px 12px;font-family:Arial,Helvetica,sans-serif;">
    <table role="presentation" align="center" width="480" cellpadding="0" cellspacing="0" border="0"
      ...
      <tr><td style="padding:12px 28px 0;font-size:14px;line-height:1.6;">
        ...
        <div style="color:#71717a;margin-top:6px;">Booking ${esc(formatTicketNo(ticket?.ticketId, ticket?.createdAt))}</div>
      </td></tr>
  ...
  const text =
    `Booking cancelled (Booking ${formatTicketNo(ticket?.ticketId, ticket?.createdAt)})\n` +
    `Event: ${event?.title}\nWhen: ${fmtDay(event?.start_datetime)} ${fmtClock(event?.start_datetime)}\n\n` +
    (refund
