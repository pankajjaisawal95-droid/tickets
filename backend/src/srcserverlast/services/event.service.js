...
export const getTicketTypesByEventService = async (eventId, userId = null) => {
  const [rows] = await pool.query(
    `
    SELECT
      tt.id,
      ...
      -- how many of THIS type the logged-in user already holds (live HOLD +
      -- PAID), counted across all their orders. 0 for guests. Lets the UI show
      -- the per-user cap up front and disable "add" instead of failing later.
      IFNULL((
        SELECT SUM(uoi.quantity)
        FROM order_items uoi
        JOIN orders uo ON uo.id = uoi.order_id
        WHERE uoi.ticket_type_id = tt.id
          AND uo.user_id = ?
          AND ( uo.status = 'PAID'
                OR (uo.status = 'HOLD' AND uo.hold_expires_at > NOW()) )
      ), 0) AS user_held,
