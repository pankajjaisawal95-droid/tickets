...
import { attachTicketToOrderAddress } from "./orderAddress.service.js";
...
export const createTicketService = async (orderId) => {
  ...
  try {
    ...
    if (existing) {
      await conn.commit();
      // Link the existing ticket to the order's address record (background-safe).
      await attachTicketToOrderAddress(orderId, existing.id);
      return {
    ...
    /* Link the generated ticket to the order's address record (background-safe). */
    await attachTicketToOrderAddress(orderId, ticketId);
...
export const cancelTicketWithRefundService = async (ticketId, userId, reason = null) => {
  ...
  const [[t]] = await pool.query(
    `SELECT t.id, t.order_id, t.user_id, t.status, t.event_id, t.used_ticket, t.created_at,
  ...
  sendCancellationConfirmation(t.order_id, t.event_id, userId, ticketId, event, refund, t.created_at);
 ...
 */
const sendCancellationConfirmation = async (orderId, eventId, userId, ticketId, event, refund, createdAt = null) => {
  try {
    const [[contact]] = await pool.query(
      `SELECT name, email, whatsapp_no
       FROM event_user_detail
