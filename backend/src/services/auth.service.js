...
export const getSavedEventUserDetail = async (userId) => {
  const [rows] = await pool.query(
    `
    SELECT name, city, whatsapp_no, email
...
export const upsertEventUserDetail = async (data) => {
  ...
  try {
    const { user_id, name, city, mobile, email, event_id, sync_user_data } = data;
    ...
    if (existing.length > 0) {
      ...
      const [result] = await pool.query(
        ...
        (user_id, name, city, whatsapp_no, email, event_id, sync_user_data, creation_time, updation_time, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `,
        [
          user_id,
...
export const insertEventUserDetail = async (data) => {
  try {
    const { user_id, name, city, mobile, email, event_id } = data;
    const currentTime = new Date();
    ...
    const [result] = await pool.query(
      `
      INSERT INTO event_user_detail
      (user_id, name, city, whatsapp_no, email, event_id, creation_time, updation_time, status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
      [
        user_id,
