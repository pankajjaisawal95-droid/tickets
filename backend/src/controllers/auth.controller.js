...
/**
 * GET /auth/profile
 * Returns the logged-in user's saved name + email + mobile (for prefilling the
 * "My Profile" form). The login/OTP response only carries tokens, so the client
 * needs this to show the current values.
 */
export const getProfile = async (req, res, next) => {
  try {
    ...
    const { name, email, mobile } = userResult.user;
    return success(res, { name: name || '', email: email || '', mobile }, 'Profile fetched');
  } catch (err) {
    next(err);
  }
};
...
export const addEventUserDetail = async (req, res, next) => {
  try {
    const { name, city, email, mobile, event_id, sync_user_data } = req.body;
